...
if(isset($_REQUEST['idch']))
	$idCargaHoraria=$_REQUEST['idch'];
else
	$idCargaHoraria= $_SESSION['idCargaHoraria'];
...
concat(nombre,' ', IFNULL(apellidoPaterno,''),' ',IFNULL(apellidoMaterno,'')) as nombre,
   PLACVE, clavecar,EXTCVE,SISCVE ,PAQCVE, descripcion, asignatura, fechaparcial$numparcial,
...
$fechaparcial = $filas1[$numfecha];
 $consulta2 	= "select distinct alumnos.matricula, nombre, concat(IFNULL(apellidoPaterno,''),' ',IFNULL(apellidoMaterno,'')) as apellido,
   email, ordinario1, ordinario2, ordinario3, calificacion.cveasignatura, cargahorarias.clavemateria,
